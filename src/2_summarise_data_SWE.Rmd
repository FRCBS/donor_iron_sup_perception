---
title: "Donor iron supplement perception data summary SWE"
...
#run code below in console:
 # rmarkdown::render('L:/DonorMedicin/FORTE/E Internation Donor Perception Survey/B Analysis/R/1. R Scripts/src/2_summarise_data_SWE.Rmd', clean=TRUE,output_format='html_document',output_file='L:/DonorMedicin/FORTE/E Internation Donor Perception Survey/B Analysis/R/4. Results/summarise_data_SWE.html') 
...
```{r}
#data <- readRDS("L:/DonorMedicin/FORTE/E Internation Donor Perception Survey/B Analysis/R/2. Datasets/1. iDPS - Summary data - JPN.RDS")
data <- readRDS("L:/DonorMedicin/FORTE/E Internation Donor Perception Survey/A Source documents/iDPS - SWE/dataSWE.rds")
...
questions <- questions %>% filter(SWE == "x")
...
This question was not included in Sweden.
```{r , eval=FALSE}
items = data %>% 
...
```{r}
items <- data %>%
  # Select columns matching the pattern "LackDeferralInfo[0-9]"
  select(which(str_detect(colnames(data), "^LackDeferralInfo[0-9]$"))) %>%
  # Convert columns to character (to safely replace NAs)
  mutate_all(~ as.character(.)) %>%
  # Replace NA values with "FALSE" (character)
  mutate_all(~ replace_na(., "FALSE")) %>%
  # Convert columns back to logical TRUE/FALSE
  mutate_all(~ as.factor(as.character(.)))
...
1= decrease motivation, 3 = no effect, 5 = increase motivation
...
```{r , warning=FALSE}
items = data %>% 
    select(which(str_detect(colnames(data) ,"^DeferralSource[0-9]_Confidence"))) 
...
```{r, eval=FALSE}
items = data%>% 
    select(which(str_detect(colnames(data) ,"^IronSuppSanqReason$"))) %>% mutate_all(~as.factor(.)) %>% mutate_all(~ factor(., levels = c("To improve my health","To be able to continue to donate","Different reason","I would never make use of iron supplements provided by the blood service"), labels = c("To improve my health","To be able to continue to donate","Different reason","I would never make use of iron supplements provided by the blood service")))
