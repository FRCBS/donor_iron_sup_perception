...
tmp <- data %>% 
  ...
  mutate_at(vars(which(str_detect(colnames(data) ,"^OpinionBloodService[0-9]$|^IronSuppSanq[0-9]$"))), ~ f1(as.character(.)) ) %>% 
  ...
  mutate_at(vars(which(str_detect(colnames(data), "^Deferral_Motivation[0-9]$"))), ~ f14(as.character(.)))  %>%
...
saveRDS(DataExport,file="L:/DonorMedicin/FORTE/E Internation Donor Perception Survey/B Analysis/R/2. Datasets/individual/1. iDPS - Summary data - JPN.RDS")
