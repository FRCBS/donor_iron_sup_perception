...
#rmarkdown::render("L:/DonorMedicin/FORTE/E Internation Donor Perception Survey/B Analysis/R/1. R Scripts/src/5_summarise_data_across_countries.Rmd", clean=TRUE,output_format='html_document',output_file='L:/DonorMedicin/FORTE/E Internation Donor Perception Survey/B Analysis/R/4. Results/summarise_data_across_countries.html')
...
plot_type_3a <- function(qdata=qdata, folder_name=FolderName, VariableName, free_x=T, figwidth=9, figheight=7, legendtitle=legendtitle) {
  p <- ggplot(qdata) + 
    theme_bw() +
    geom_col(aes(x = answer, y = Percentage, fill = interaction(Country, VariableGroup)), position = position_dodge(width = 0.9))  
  ...
  p <- p + ylab("Percentage") +
    ...
    ggtitle(str_wrap(unique(qdata$Q)[1], width = 75)) +
    scale_y_continuous(breaks = seq(0, 100, by = 50), limits = c(0, 120), position = "right") +
    scale_fill_manual(name = legendtitle, breaks = c("USA.hb", "USA.fer"), labels = c("Hb", "Ferritin"), values = c(
      "Netherlands.other" = "#E69F00", 
      "France.other"      = "#0072B2", 
      "Sweden.other"      = "#F0E442", 
      "Finland.other"     = "#56B4E9", 
      "Japan.other"       = "#D55E00",  
      "Germany.other"     = "#CC79A7", 
      "USA.hb" = "#B3B300",
      "USA.fer" = "#808000"
    )) +
    # Enable the legend to check grouping
    # guides(fill = FALSE) +
    theme(strip.text.y.left = element_text(angle = 0),
          strip.placement = "outside") +
    geom_text(aes(x = answer, y = Percentage, label = round(Percentage, 1), 
                  group = interaction(Country, VariableGroup)), 
              vjust = -1.5, size = 2, color = "black", position = position_dodge(width = 0.9)) +
    scale_x_discrete(labels = function(x) str_wrap(x, width = 10))
  
  print(p)

  # Save plot with increased size
  filename <- paste0(folder_name, "/", VariableName, " - by country.png")
  save_plot(p, filename, width = figwidth + 2, height = figheight + 2)
}
...
plot_type_4 <- function(qdata=qdata, country_colors=country_colors, folder_name=FolderName, VariableName, free_y=T,figwidth=9, figheight=7, Qtitle=T){
  ...
  if(free_y){
    p<- p+ facet_grid(Statement ~ Country, 
             labeller = labeller(Statement = label_wrap_gen(width = 30, multi_line = TRUE)),
             switch = "y",
             scales="free_y") 
    } else {
          p<- p+ facet_grid(Statement ~ Country, 
...
country_colors <- c(
  ...
  "Germany"     = "#CC79A7", # Purple (Colorblind-friendly purple)
  "USA" = "#B3B300" # Olive (Colorblind-friendly olive?)
)
...
qdata <- merge(q, question_text, by.x = "Question", by.y = "Variable", all.x = TRUE) %>%  filter(Response %in% c("18-20", "17-20", "21-30", 
                                          "31-40", "41-50", 
                                          "51-60", "61-70", "71-80","18-19","20-29","30-39","40-49","50-59","60-69","70-79", "80+")) %>%
  mutate(
...
qdata <- merge(q, question_text, by.x = "Question", by.y = "Variable", all.x = TRUE) %>%   
  mutate(
    #Statement = sub(".*?, then (.*)", "\\1", Question_Text),
    #Q = "Are the following statements True or False?", #sub("(.*), then.*", "\\1", Question_Text),
    answer = factor(trimws(Response), levels = c("I don’t know", "No", "Yes, multiple times", "Yes, once", "Yes"))
  )%>% 
  droplevels()
...
q <- data %>% filter(grepl("DeferralSource[1-7](A|.NL|_ARC)?$", Question))
...
qdata <- merge(q, question_text, by.x = "Question", by.y = "Variable", all.x = TRUE) %>%   
  ...
  ) %>% filter(Response == "Yes" | Response == "TRUE") %>% mutate(Statement = ordered(Statement, levels = c(" Blood service staff", " Blood service website and/or brochure", " Donor physician"," Health care practitioner", " Search information on the internet", " Friend or family member", " Other, namely", " None")))%>% 
  droplevels()
...
```{r, echo=F,fig.width = 9, fig.height = 7}
q <- data %>% filter(grepl("^DeferralSource[1-9](_ARC)?_Confidence.FI$", Question))

qdata <- merge(q, question_text, by.x = "Question", by.y = "Variable", all.x = TRUE) %>%   
  mutate(
    Response = trimws(as.character(Response)),
    Statement = sapply(str_split(Question_Text, "[:?]"), function(x) x[2]),
    Statement = sapply(Statement, function(x) str_remove(x, "confidence |Confidence ")),
  ...
  )%>% mutate(Statement = ordered(Statement, levels = c(" Blood service staff", " Blood service website and/or brochure", " Donor physician"," Health care practitioner", " Search information on the internet", " Friend or family member", " Other, namely"))) %>%
  droplevels()
...
```{r, echo=F,fig.width = 9, fig.height = 7}
q <- data %>% filter(grepl("^DeferralInfo(_hb|_lowFer)?[1-9]$", Question))
...
qdata <- merge(q, question_text, by.x = "Question", by.y = "Variable", all.x = TRUE) %>%   
  mutate(
    ...
    Q = sapply(str_split(Question_Text, "[:?]"), function(x) x[1]),
    answer = ordered(Response, levels=c(1,2,3,4,5))
  )%>% mutate(VariableGroup = case_when(
    grepl("^DeferralInfo_hb", Question) ~ "hb",
    grepl("^DeferralInfo_lowFer", Question) ~ "fer",
    TRUE ~ "other"
  )) %>% arrange(country)%>%
  droplevels()

plot_type_3a(qdata=qdata, folder_name=FolderName, VariableName="DeferralInfo", free_x=F,legendtitle="Deferral reason")
...
```{r, echo=F,fig.width = 9, fig.height = 7}
q <- data %>% filter(grepl("^LackDeferralInfo(_hb[1-5]|_lowFer[1-5]|[1-5])$", Question))

qdata <- merge(q, question_text, by.x = "Question", by.y = "Variable", all.x = TRUE) %>%   
  mutate(
    Statement = sapply(str_split(Question_Text, "[:?]"), function(x) x[2]),
    Q = sapply(str_split(Question_Text, "[:?]"), function(x) x[1]),
    answer = ordered(Response, levels=c("TRUE", "FALSE", "Yes", "No", "1", "0"))
  ) %>% filter(Response == "Yes" | Response == "TRUE" | Response =="1") %>% 
  droplevels()
...
qdata <- merge(q, question_text, by.x = "Question", by.y = "Variable", all.x = TRUE) %>%   
  mutate(
    Statement = Response,
    Q = sapply(str_split(Question_Text, "[:?]"), function(x) x[1]),
    answer = ordered(Response, levels=c("Yes", "No", "I don't know", "Never", "Not anymore", "Sometimes", "Every day"))
  ) %>% 
  droplevels()
...
plot_type_4(qdata=qdata, country_colors = country_colors, folder_name=FolderName, VariableName="IronSuppEffect", free_y=T, figheight=10, Qtitle=T)
