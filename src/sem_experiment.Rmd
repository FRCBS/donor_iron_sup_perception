...
First item needs to be positively correlated with the latent variable!
Or we need constrain the variation of the latent variable to 1.

https://lavaan.ugent.be/tutorial/syntax1.html

variables (starting with an ‘f’ below) may be latent. For example:
...
summary(fit, standardised=TRUE) # should we standardise all?

then std.all is correlation

factor loadings should above .3?

est.std = loading 

rmsea should be 0.08 or lower?

variance explained

inspect(fit, "r2")

why do they specify that something can correlate?
It improves the model maybe, but who do you choose what can correlate?

standardised residuals tell you are somethings more correlated than the model fit allows.

modification indices, how much you correlation
with in latent factor would make sense
the larger the mi the better!

predict gives you the scores per person to be used elsewhere
