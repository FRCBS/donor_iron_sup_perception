...
names(data) <- gsub("Trust about the information from the EFS", "DeferralSource1_Confidence.FI", names(data))
names(data) <- gsub("From you GP", "DeferralSource2_Confidence.FI", names(data))
names(data) <- gsub("From BS website", "DeferralSource3_Confidence.FI", names(data))
names(data) <- gsub("From other internet sources", "DeferralSource4_Confidence.FI", names(data))
names(data) <- gsub("Others", "DeferralSource5_Confidence.FI", names(data))
...
names(data) <- gsub("Do you have any concerns about iron supplements", "Concern_IronSupp", names(data))
names(data) <- gsub("Do you have other concerns about iron supplements", "Concern_IronSupp_Other", names(data))
names(data) <- gsub("If yes, what cooncerns", "Concern_IronSupp_Other_Text", names(data))
...
data <- data %>%
  mutate(
    # Create binary columns for each predefined answer option using regex to match the content
    ConcernIronSupp1 = sapply(as.character(Concern_IronSupp), function(x) ifelse(grepl("I dont know if iron supplements can be harmfull", x), 1, 0)),
    ConcernIronSupp2 = sapply(as.character(Concern_IronSupp), function(x) ifelse(grepl("I will not take iron supplements unless I have symptoms", x), 1, 0)),
    ConcernIronSupp3 = sapply(as.character(Concern_IronSupp), function(x) ifelse(grepl("I think that iron supplements are harmfull", x), 1, 0)),
    ConcernIronSupp4 = sapply(as.character(Concern_IronSupp), function(x) ifelse(grepl("I prefer to eat more iron rather than taking iron supplements", x), 1, 0)),
    ConcernIronSupp5A = sapply(as.character(Concern_IronSupp_Other), function(x) ifelse(grepl("YES", x), 1, 0)),
    
        ConcernIronSupp5B = Concern_IronSupp_Other_Text
  )

data <- data %>% select(-c("Concern_IronSupp", "Concern_IronSupp_Other", "Concern_IronSupp_Other_Text"))
...
```{r}
tmp <- tmp %>%
  select(-c("SupplementsUse_Type_A","SupplementsUse_Type_B","SupplementsUse_Type_C","AccForIronLoss4B","LackDeferralInfoTEXT","DeferralSource6B","SupplementsReason6B","IronSuppSanqReasonB","ConcernIronSupp5B","IronSuppEffect6B", "SupplementsOpen1", "SupplementsOpen2"))
```
...
saveRDS(DataExport,file="L:/DonorMedicin/FORTE/E Internation Donor Perception Survey/B Analysis/R/2. Datasets/individual/1. iDPS - Summary data - FR.RDS")
